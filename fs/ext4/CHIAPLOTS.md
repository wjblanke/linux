...
- When no room to create another file, sleep **`INTERVAL_SEC`** seconds (default **10**); while room exists, it immediately attempts another create. If **`CHIAPLOTS_DIR`** exists (default **`/.chiaplots`**) and is writable:
  ...
  - If **metric > `THRESHOLD_MB` × 1024²** bytes (default **`THRESHOLD_MB=2048`** → **2 GiB**), first runs **`chia plotters chiapos -k 25 --override-k`** (default **k**; see **`CHIA_PLOT_K`**) in a **`mktemp`** directory under **`TMPDIR`**, then **`mv`** the finished **`.plot`** to **`CHIAPLOTS_DIR/auto_<epoch>_<pid>_<seq>_k25.plot`**. If **`chia`** is missing, keys are unavailable, plotting fails, or **`mv`** fails, it falls back to **`FILE_MB`** MiB (default **50**) with **`dd`** into **`mktemp`** and **`mv`** to **`…/auto_….bin`**. Successful creates print a timestamped **`plotpoll:`** line; failures print a short reason before fallback.
